...
public class ${CLASSNAME} extends com.matrixone.apps.program.Task
{
			...
			try
			{
				...
				if (null != businessGoalList && businessGoalList.size()>0)
				{
					...
					while(businessGoalItr.hasNext())
					{
						...
						String businessGoalId = (String) businessGoalMap.get(DomainConstants.SELECT_ID);
						String businessGoalState = (String) businessGoalMap.get(DomainConstants.SELECT_CURRENT);
			...
			if (parentList.size() > 0)
			{
				...
				//and Task is promoted.
					...
					if(dObj.isKindOf(context, TYPE_PROJECT_SPACE)){
						...
						String previousState = JPO.invoke(context, "emxProgramCentralUtilBase", null, "getPreviousState",arrJPOArguments,String.class);
						...
						if(fromState.equals(STATE_PROJECT_TASK_REVIEW) && businessGoalState.equals(STATE_BUSINESS_GOAL_ACTIVE))
						{
							...
							if (null != projectList && projectList.size()>0)
							{
								...
								while(projectItr.hasNext())
								{
									...
									String projectState = (String) projectMap.get(DomainConstants.SELECT_CURRENT);
	...
	public int triggerCheckChildrenStates(Context context, String[] args)
		...
		else
		{
			...
			//**Start**
			if(ProjectManagement.isEPMInstalled(context))
			{
				...
				if (sEvent.equals("Promote"))
				{
					...
					if (fromState.equals(STATE_PROJECT_TASK_ACTIVE)) //Added:nr2:For stage Gate highlight
					{
						...
						else
						{
							...
							taskName = taskObj.getName(context);
	...
	public int triggerCheckDependency(Context context, String[] args) throws Exception
	{
		...
		{
			String sErrMsg = "";
			String strTranslatedRequiredStateName = i18nNow.getStateI18NString(DomainConstants.POLICY_PROJECT_TASK, requiredState, context.getSession().getLanguage());
	...
	public Object getCompletedWBSTask(Context context, String[] args)
		...
		com.matrixone.apps.program.Task task = (com.matrixone.apps.program.Task) DomainObject.newInstance(context,
				DomainConstants.TYPE_TASK, "PROGRAM");
		String busWhere = DomainConstants.SELECT_CURRENT + "=='" + STATE_PROJECT_TASK_COMPLETE +"'";
	...
	public Vector getStatusIcon(Context context, String[] args)
		...
		try
		{
			...
			busSelect.add(ProjectManagement.SELECT_PERCENT_COMPLETE);
			...
			for (i = 0; i < actionListSize && actionList != null; i++){
				...
				String strPercentComplete = (String) objectMap.get(ProjectManagement.SELECT_PERCENT_COMPLETE);
				...
				if(strObjectType.equalsIgnoreCase(DomainConstants.TYPE_PERSON) || UIUtil.isNullOrEmpty(taskEstFinishDate)){
					statusGif = DomainConstants.EMPTY_STRING;
				...
				String actualFinishDateString   =   (String) objectMap.get(ProjectManagement.SELECT_TASK_ACTUAL_FINISH_DATE);
				...
				{
					daysRemaining = Task.computeDuration(sysDate,
							estFinishDate);
				}
				else
				{
	...
	public Vector getQuestionIcon(Context context, String[] args)
		...
		while (objectListIterator.hasNext())
		{
			...
			String taskLevel = (String) objectMap.get(DomainConstants.SELECT_LEVEL);
			StringList busSelects = new StringList(2);
			...
			if (!thisObjectType.equals(templateType))
			{
				...
				if(((String)map.get(Task.SELECT_HAS_QUESTIONS)).equalsIgnoreCase("true"))
				{
					...
					statusGif+=XSSUtil.encodeForHTML(context,jsTreeID);
		...
		try
		{
			...
			for (i = 0; i < actionListSize && actionList != null; i++)
			{
				...
				else
				{
					...
					taskProjectName = "<img src=\"../common/images/"+iconImage+"\"/>" + XSSUtil.encodeForXML(context,taskProjectName);
			  ...
			  try {
				...
				if(program instanceof StringList){
					...
					if(ProgramCentralUtil.isNullString(programName) || "#DENIED!".equals(programName)){
						programName = DomainConstants.EMPTY_STRING;
					}else{
	...
	public Vector getName (Context context, String[] args) throws Exception{
		...
		while (objectListIterator.hasNext()){
			...
			String critcalTask = taskInfo.get(SELECT_ATTRIBUTE_CRITICAL_TASK);
			...
			sRetValue.append(XSSUtil.encodeForXML(context, taskObj.getName()));
			if(!isPrinterFriendly){
				sRetValue.append("</a>");
			}
	...
	public void updateTaskActualFinishDate(Context context, String[] args) throws Exception
	{
		...
		if(null==locale)
		{
			Object objLocale = requestMap.get("localeObj");
			if(null!=objLocale)
			{
		...
		if (!strNewVal.equals(strOldVal)) {
			...
			if (task.isKindOf(context,DomainConstants.TYPE_PROJECT_CONCEPT)) {
			...
			Map taskData = task.getInfo(context,busSelect);
			String taskName =  (String)taskData.get(DomainConstants.SELECT_NAME);
	...
	public void updateTaskPercentageComplete(Context context, String[] args) throws Exception
	{
		...
		if (isSummaryTask)
		{
			...
			if(ProgramCentralUtil.isNullString(strTaskName)){
				strTaskName = task.getInfo(context, DomainConstants.SELECT_NAME);
			}
		...
		com.matrixone.apps.common.Task.updateDates(context, objectList, false, false);
	...
	public Vector getAssigneeFullName(Context context, String[] args)
		...
		String riskType = DomainConstants.TYPE_RISK;
		...
		String type = (String) taskInfo.get(DomainConstants.SELECT_TYPE);
		String owner = (String) taskInfo.get(DomainConstants.SELECT_OWNER);
		...
		while (objectListIterator.hasNext())
		{
			...
			String fullName = (String) objectMap.get(DomainConstants.SELECT_NAME);
	...
	public void applyMassAssignment ( Context context , String args[] )
			...
			if ( sChangeType.equals( "NEW_OWNER" ) ) {
				...
				if ((sRelPersonTaskId != null ) && (!sRelPersonTaskId.equals("null")))
				{
					DomainRelationship relationship = new
							DomainRelationship(sRelPersonTaskId);

					String rel_allocation=PropertyUtil.getSchemaProperty(context,"attribute_PercentAllocation");
					ContextUtil.pushContext(context);
					try
					{
		...
		while (keyMapIter.hasNext())
		{
			...
			objectIdList.clear();
			sFinalMailMessage = DomainConstants.EMPTY_STRING;
			// Set the "to" list.
			DomainObject personObject = DomainObject.newInstance(context, personId);
			String personName = personObject.getName(context);
			mailToList.clear();
			...
			for(int i= 0; i < values.size(); i++) {
				...
				Map taskMap = dom.getInfo(context,sList);
	...
	public StringList getStates(Context context, String[] args) throws Exception {
		...
		Map map = dom.getInfo(context,sList);
		...
		if(sType.equals(DomainConstants.TYPE_PROJECT_SPACE)){
			...
			for(;;){
				Map childMap = dom.getRelatedObject(context,
						relType,
						true,
						sList,
						null);
				if(childMap!=null && childMap.size()!=0) {
					if(((String)childMap.get(DomainConstants.SELECT_TYPE)).equals(DomainConstants.TYPE_TASK)){
						...
						StringList subStateList = dom.getInfoList(context,DomainConstants.SELECT_STATES);
						// Task Review to be removed since Project Concept having Review state
						subStateList.remove(DomainConstants.STATE_PROJECT_SPACE_REVIEW);
	...
	public MapList getWBSSubtasks(Context context, String[] args) throws Exception
	{
		...
		{
			mapList = getWBSTasks(context,strObjectId,DomainConstants.RELATIONSHIP_SUBTASK,nExpandLevel);
		}
	...
	public MapList getWBSDeletedSubtasks(Context context, String[] args) throws Exception {
		...
		if(task.isKindOf(context, DomainConstants.TYPE_PROJECT_SPACE)||task.isKindOf(context, DomainConstants.TYPE_PROJECT_CONCEPT)){
			...
			if(selectedTable.equalsIgnoreCase("PMCProjectTaskEffort")){
				...
				programMap.put("objectId", strObjectId);
				arrJPOArguments = JPO.packArgs(programMap);
				wbsDeletedTasks = JPO.invoke(context,
						"emxEffortManagementBase", null, "getProjectTaskList",
						arrJPOArguments, MapList.class);
			}
			else {
...
/*                wbsDeletedTasks = task.getRelatedObjects(   context,
	...
	public MapList getWBSAllSubtasks(Context context, String[] args) throws Exception {
		...
		if(task.isKindOf(context, TYPE_TASK_MANAGEMENT) && selectedTable.equalsIgnoreCase("PMCProjectTaskEffort")){
			...
			programMap.put("objectId", strObjectId);
			arrJPOArguments = JPO.packArgs(programMap);
			wbsSubTasks = JPO.invoke(context,
					"emxEffortManagementBase", null, "getProjectTaskList",
					arrJPOArguments, MapList.class);
		}//Modified:6-Oct-2010:R211:RG6:IR-072844V6R2012
	...
	public Vector getStateName(Context context, String[] args) throws Exception {
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				String taskId = (String) mapObject.get(DomainConstants.SELECT_ID);
			...
			BusinessObjectWithSelectList objectWithSelectList = BusinessObject.getSelectBusinessObjectData(context, strObjectIds, slBusSelect);
	...
	public HashMap getProjectMembers(Context context, String[] args) throws Exception {
		...
		try {
			...
			if(taskSubtypes.indexOf(strType)>=0){
				Map mainMap;
				dom.setId(strObjectId);
				strTaskProjectRole = dom.getInfo(context,"attribute["+DomainConstants.ATTRIBUTE_PROJECT_ROLE+"]");
				...
				if(mainList != null && mainList.size() >0){
					...
					mainMap = dom.getInfo(context,typeSelects);
				}
			...
			String projectVisibility = project.getInfo(context,ProjectSpace.SELECT_PROJECT_VISIBILITY);
			...
			while (itr.hasNext()) {
				...
				if(!memberList.contains(fullName)){
					memberList.add(fullName);
					personList.add((String) mapPersonName.get(DomainConstants.SELECT_ID));
				}
			}
	...
	private HashMap getParentProjectMap(Context context,DomainObject dom) throws Exception {
		...
		if(mainList != null && mainList.size() >0){
			...
			mainMap = dom.getInfo(context,sList);
		}
	...
	{
		char cFieldSep = 0x07;
		String value = bws.getSelectData(selectable);
	...
	public Object getTaskDependencyColumn(Context context, String[] args) throws Exception
	{
		...
		for (int i=0; i < objectList.size(); i++)
		{
			...
			String taskId = (String) map.get(DomainConstants.SELECT_ID);
			taskIds[i] = taskId;
		}
			...
			if (predecessorIds.size() > 0 && !"".equals(((String)predecessorIds.get(0)).trim()))
			{
				...
				for (int i=0; i < depedencyList.size(); i++)
				{
					...
					if (predecessorProjectType == null)
					{
						...
						StringList projectInfo = StringUtil.split(output, "|");
	...
	public Object getTaskSuccessorColumn(Context context, String[] args) throws Exception
	{
		...
		String IS_SUCCESSOR_KINDOF_PROJECT = "to["+RELATIONSHIP_DEPENDENCY+"].from.type.kindof["+DomainConstants.TYPE_PROJECT_SPACE+"]";
	...
	public Vector getNameColumnData (Context context,String mode, String[] args) throws Exception
	{
		...
		String IS_SUMMARY_TASK = "from["+DomainConstants.RELATIONSHIP_SUBTASK+"].to.id";
	...
	private void updateBudgetDates(Context context, String objectId, String projectId, String projectBudgetId) throws Exception {
		...
		attributes.put(ProgramCentralConstants.ATTRIBUTE_COST_INTERVAL_START_DATE,projectEstStartDate);
		attributes.put(ProgramCentralConstants.ATTRIBUTE_COST_INTERVAL_END_DATE,projectEstFinishDate);
	...
	public void updateEstEndDate(Context context, String[] args) throws Exception {
		...
		String projectBudgetId 		= projectBwsl.getSelectData(ProgramCentralConstants.SELECT_PROJECT_FINANCIAL_ITEM_ID);
		String projScheduledFrom	= projectBwsl.getSelectData(SELECT_ATTRIBUTE_SCHEDULED_FROM);
		String projStartDate 		= projectBwsl.getSelectData(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_START_DATE);
		String projectEndDate 		= projectBwsl.getSelectData(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_FINISH_DATE);
		...
		String taskCalendarId 	= tasktBwsl.getSelectData(ProgramCentralConstants.SELECT_CALENDAR_ID);
		String taskCalendarDate = tasktBwsl.getSelectData(ProgramCentralConstants.SELECT_CALENDAR_DATE);
		String taskStartDate 	= tasktBwsl.getSelectData(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_START_DATE);
	...
	public boolean hasAccessForWBSView(Context context, String args[]) throws Exception {
		...
		if(DomainConstants.TYPE_PROJECT_TEMPLATE.equalsIgnoreCase(objType) ||
			...
			ProjectTemplate projectTemplate = (ProjectTemplate)DomainObject.newInstance(context, DomainConstants.TYPE_PROJECT_TEMPLATE, DomainConstants.PROGRAM);
	...
	public boolean hasAccessForCreateBaseline(Context context, String args[]) throws Exception
	{
		...
		Map objectMap = project.getInfo(context,sList);
	...
	public boolean hasAccessToBaselineView(Context context, String args[]) throws Exception
	{
		...
		Map objectMap = project.getInfo(context,objectSelect);
		String strBaselineInitialStartDate  = (String)objectMap.get(ProjectSpace.SELECT_BASELINE_INITIAL_START_DATE);
	...
	public HashMap preProcessCheckForEdit (Context context, String[] args) throws Exception
	{
		...
		String sPrjState = DomainConstants.EMPTY_STRING;
	...
	public Vector getWBSRowState(Context context, String[] args)
		...
		while (objectListIterator.hasNext())
		{
			...
			Map map = domainObject.getInfo(context,busSelects);
	...
	public Object getTaskDependencies(Context context, String[] args)
		...
		try
		{
			...
			{
				busSelects.add(SELECT_NAME);
				busSelects.add("to[" + RELATIONSHIP_SUBTASK + "]." + SubtaskRelationship.SELECT_TASK_WBS);
			...
			relSelects.add(DependencyRelationship.SELECT_LAG_TIME);
			...
			for(int i=0;i<dependencyList.size();i++){
				...
				String dependentTaskId = (String)dependencyMap.get(DomainConstants.SELECT_ID);
			...
			while (listItr.hasNext()) {
				...
				if (dependencyList.isEmpty()) {
					...
					while(dependencyListItr.hasNext()) {
						...
						lagTime = (String) dependencyMap.get(DependencyRelationship.SELECT_LAG_TIME);
						...
						if(dependencyId.equals(taskId)) {
							dependencyType = (String) dependencyMap.get(DependencyRelationship.SELECT_DEPENDENCY_TYPE);
							break;
						} else {
			...
			taskList.sort("to[" + RELATIONSHIP_SUBTASK + "]." + SubtaskRelationship.SELECT_TASK_WBS, "ascending", "multilevel");
		}catch(Exception exc) {
		}
		return taskList;
			} //end of method.
	...
	public StringList getTaskDependentProject(Context context, String[] args)throws Exception {
		...
		for(int i=0;i<taskList.size();i++){
			...
			taskIdArray[i] = taskMap.get(DomainConstants.SELECT_ID);
		}
	...
	public MapList getWBSIndependentTaskList(Context context, String[] args)
		...
		try{
			...
			short nExpandLevel =  ProgramCentralUtil.getExpandLevel(strExpandLevel);
			taskMapList = getWBSTasks(context,strProjectId,DomainConstants.RELATIONSHIP_SUBTASK,nExpandLevel);
			com.matrixone.apps.program.Task task = (com.matrixone.apps.program.Task) DomainObject.newInstance(context, DomainConstants.TYPE_TASK, "PROGRAM");
			...
			for (Iterator itrRelatedObjects = taskParents.iterator(); itrRelatedObjects.hasNext();)
			{
				...
				String strParentTaskId= (String)mapRelatedObjectInfoParent.get(DomainConstants.SELECT_ID);
			...
			for (Iterator itrRelatedObjects = taskChildren.iterator(); itrRelatedObjects.hasNext();)
			{
				...
				String strChildTaskId= (String)mapRelatedObjectInfoChildren.get(DomainConstants.SELECT_ID);
		...
		while (objectListIterator.hasNext()){
			...
			if("0".equals(taskLevel) && !isModifyOp)
				...
				if(isPrinterFriendly){
					...
					for (int i = 0; i < unitList.size(); i++) {
						unit = (Unit)unitList.get(i);
						String lable=DomainConstants.EMPTY_STRING;
	...
	public String getActualDuration(Context context, String args[])throws Exception {
		...
		String taskActualDuration = task.getInfo(context,ProjectManagement.SELECT_TASK_ACTUAL_DURATION);
	...
	private StringList getAllProjectSubTypeNames(Context context, String type) throws FrameworkException
	{
		...
		subTypeList = StringUtil.split(subTypes, "|");
		return subTypeList;
	}
	...
	public void updateOwner(Context context, String[] args) throws Exception {
		...
		String newOwner = assignee.getInfo(context, DomainConstants.SELECT_NAME);

		if (task.isKindOf(context, DomainConstants.TYPE_TASK_MANAGEMENT))
		{
			//Get the current owner's name
			String currentOwner = task.getInfo(context, DomainConstants.SELECT_OWNER);
			...
			while(assigneesListItr.hasNext()) {
				...
				if(newOwnerId.equals(assigneeMap.get(DomainConstants.SELECT_ID))) {
					assigneeInList = true;
				}
				if(currentOwner.equals(assigneeMap.get(DomainConstants.SELECT_NAME))) {
					currentOwnerRelId = (String) assigneeMap.get(DomainConstants.SELECT_RELATIONSHIP_ID);
				}
			}  //end while
		...
		if(!isProjectSpace){
			...
			while(typeMapListItr.hasNext())
			{
				...
				if ( attrName.equals( Task.ATTRIBUTE_PROJECT_ROLE) )
				{
					...
					{
						String sProjectRole = rolesList2.obj();
						// IR Fix 310707 - End
		...
		try {
			if(newValue!=null) {
				...
				while (predecessorItr.hasNext()) {
					...
					String connectionId = (String) predecessorObj.get(DomainRelationship.SELECT_ID);
					...
					String strOldValue = DomainConstants.EMPTY_STRING;
					...
					strOldValue = strOldValue +":" +(String) predecessorObj.get(DependencyRelationship.SELECT_DEPENDENCY_TYPE);
				...
				for (int i=0;i<newDependencyArray.length;i++) {
					...
					if(extProject) {
						...
						String typePatternPrj = DomainConstants.TYPE_PROJECT_SPACE + "," +DomainConstants.TYPE_PROJECT_CONCEPT;
					...
					if ((extProject && isdObjKindofTaskManagement) || isTaskManagement) {
						...
						String prjID = ProjectManagement.getParentProject(context,dObj);
						dObj.setId(prjID);
					}
					...
					attributes.put(DomainConstants.ATTRIBUTE_DEPENDENCY_TYPE,dependencyType);
					...
					if(ProgramCentralUtil.isNotNullString(connectionID)) {
						...
						String predecessorDependencyType = (String) predecessorObj.get(DependencyRelationship.SELECT_DEPENDENCY_TYPE);
						String strLagTime = (String) predecessorObj.get(SELECT_TASK_LAGTIME_VALUE);
				...
				if(deleteArray!=null && deleteArray.length>0) {
					...
					String connectionId = DomainConstants.EMPTY_STRING;
					for(int m=0 ;m<deleteArray.length;m++){
						...
						Hashtable relData = domRel.getRelationshipData(context, relationshipSelects);
				...
				if(!modifyPredMap.isEmpty()){
					...
					if(isCalledFromExperimentCompareUI){
						taskObject.modifyPredecessors(context, modifyPredMap,false);
					} else {
						taskObject.modifyPredecessors(context, modifyPredMap,true);
					}
				}
				...
				if(!addPredMap.isEmpty()){
					...
					if(isCalledFromExperimentCompareUI){
						taskObject.addPredecessors(context, addPredMap, false);
					} else {
						taskObject.addPredecessors(context, addPredMap, true);
					}
				}
	...
	public Map cutPasteTasksInWBS(Context context, String[] args) throws Exception
	{
        ...
        String strLanguage=context.getSession().getLanguage();
			...
			while(itrC.hasNext()){
				...
				String sObjectId = childCElement.getAttributeValue("objectId");
				String sRelId = childCElement.getAttributeValue("relId");
				String sRowId = childCElement.getAttributeValue("rowId");
				String markup = childCElement.getAttributeValue("markup");
				String spasteAbove = childCElement.getAttributeValue("paste-above");
				...
				if (spasteAbove != null){
					StringList objectList = StringUtil.split(spasteAbove,"|");
    			...
    			if ("resequence".equals(markup)){
					...
					com.matrixone.apps.common.Task.disconnectTasks(context, new String[]{sRelId}, sParentOID);
...
/*			String immediateProjectId = null;
			...
			typePattern = DomainConstants.TYPE_PROJECT_MANAGEMENT;
			getFrom = true;
			strDirection = "from";
			mapList = rootNodeObj.getRelatedObjects(context,
					relPattern,
	...
	public String getCalendarName(Context context, String args[]) throws Exception
	{
		...
		String popTreeUrl = "";
		if(calendar != null){
			strCalendarName = XSSUtil.encodeForHTML(context,calendar.getName());
 ...
 throws Exception
 {
	...
	public boolean updateTaskConstraintType(Context context,String args[]) throws Exception{
		try {
			...
			com.matrixone.apps.common.Task.updateDates(context, dateList, true, false);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static MapList getPMCWBSPlanningViewTable(Context context, String[]args) throws FrameworkException{
		...
		try {
			...
			mlColumns = UITableCustom.getColumns(context,"PMCWBSViewTable",null);
			String languageStr = context.getSession().getLanguage();
			...
			StringList slExcludeColumns = StringUtil.splitString(strConstraintTypei18, ",");
	...
	public StringList getTaskDependancyDurationKeyword(Context context, String[] args)
		...
		while (objectListIterator.hasNext())
		{
			...
			for (int i = 0; i < sdurationKeyword.length; i++) {
				...
				.append(XSSUtil.encodeForHTML(context,durationKeyword.getName()))
				.append("</option>");
			}
			lagTime.append("</select>");
	...
	public HashMap getPMCTaskPolicies(Context context,String[] args) throws Exception {
		...
		try{
			...
			if(("Project Task".equalsIgnoreCase((String)taskInfo.get(SELECT_POLICY)) && ProgramCentralUtil.isNotNullString(hasProjectTask) && hasProjectTask.indexOf("Task")>=0 )){
				fieldRangeValue.add(DomainConstants.POLICY_PROJECT_TASK);
	...
	public void  updateAllocation(Context context, String[] args) throws Exception{
		...
		final String SELECT_PROJECT_ID = "to[" + DomainConstants.RELATIONSHIP_PROJECT_ACCESS_KEY + "].from.from[" + DomainConstants.RELATIONSHIP_PROJECT_ACCESS_LIST + "].to.id";
		...
		MapList taskInfoMapList = DomainObject.getInfo(context, taskIds, busSelect);
		...
		for (Iterator itrTaskInfoList = taskInfoMapList.iterator(); itrTaskInfoList.hasNext();) {
			...
			taskAssignees = StringUtil.split(assignee, matrix.db.SelectConstants.cSelectDelimiter);
		}
		...
		if(isContextUserValid && !ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE.equalsIgnoreCase(strCurrentTaskState)){
			...
			if(ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE.equalsIgnoreCase(strCurrentTaskState)){
				...
				nameSelects.add(DomainConstants.SELECT_NAME);
				...
				String strAssignee = (String)personInfoMap.get(DomainConstants.SELECT_NAME);
					...
					if (!STATE_PROJECT_TASK_ASSIGN.equals(strCurState)) {
						...
						if (blnUpdate) {
							...
							if (!"true".equalsIgnoreCase(blockValue)) {
								...
								.setAttributeValue(
										context,
										DomainConstants.ATTRIBUTE_TASK_ACTUAL_START_DATE,
										strNewStartDate);
							}
						...
						if (strActualStartDate != null
							...
							if (!"true".equalsIgnoreCase(blockValue)) {
								...
								.setAttributeValue(
										context,
										DomainConstants.ATTRIBUTE_TASK_ACTUAL_DURATION,
										duration.toString());
							}
						}
					} else {
	...
	public int triggerCheckActualFinishDate(Context context, String[] args) throws Exception
	{
		...
		if(null!=strNewVal && !"".equalsIgnoreCase(strNewVal) && !"null".equalsIgnoreCase(strNewVal))
		{
			...
			if (!strNewVal.equals(strOldVal)) {
				...
				Map taskData = task.getInfo(context, busSelect);
				String taskName = (String) taskData.get(DomainConstants.SELECT_NAME);
				String taskType = (String) taskData.get(DomainConstants.SELECT_TYPE);
				String strActualStartDate = (String) taskData.get(SELECT_ACT_START_DATE);
	...
	public int triggerUpdateRemainingEffort(Context context, String[] args) throws MatrixException {
		...
		String type_Effort=PropertyUtil.getSchemaProperty(context,"type_Effort");
		String type_TaskManagement=PropertyUtil.getSchemaProperty(context,"type_TaskManagement");
		String relPattern=PropertyUtil.getSchemaProperty(context,"relationship_hasEfforts");
		String attribute_TotalEffort=PropertyUtil.getSchemaProperty(context,"attribute_TotalEffort");
		String attribute_Duration=PropertyUtil.getSchemaProperty(context,"attribute_TaskEstimatedDuration");
		...
		try {
			...
			if("TRUE".equalsIgnoreCase(isKindofTaskManagement) && !slEffortIds.isEmpty()) {
				...
				String attribute_RemainingEffort=PropertyUtil.getSchemaProperty(context,"attribute_RemainingEffort");
	...
	public void updateWBSElementsName(Context context, String[] args) throws MatrixException {
		try {
			...
			if(ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE.equals(taskState)||
					DomainConstants.STATE_PROJECT_SPACE_ARCHIVE.equals(taskState)){
	...
	public HashMap getTaskTypes(Context context, String[] args) throws MatrixException {
		...
		for (Iterator iterator = slTaskSubTypes.iterator(); iterator.hasNext();) {
            ...
            System.out.println("*****i18nTaskTypeName******"+i18nTaskTypeName);
	...
	public HashMap getRangeForTasksToAdd(Context context, String[] args) throws MatrixException
	{
		...
		StringList slTasksToCreate = StringUtil.split(strTasksToAdd, "|");
	...
	public MapList getMembersConnectedToProject (Context context, String[] args) throws MatrixException
	{
		try {
			...
			for (Iterator iterator = ml.iterator(); iterator.hasNext();) {
				...
				String objName = (String) objectMap.get(DomainConstants.SELECT_NAME);
				...
				String userType = (String) objectMap.get(DomainConstants.SELECT_TYPE);
				String lastName = (String) objectMap.get(Person.SELECT_LAST_NAME);
				String firstName = (String) objectMap.get(Person.SELECT_FIRST_NAME);
			...
			for (Iterator iterator2 = mlTaskAssigneeInfo.iterator(); iterator2.hasNext();) {
				...
				String objId = (String) mpPersonDetails.get(DomainConstants.SELECT_ID);
	...
	public Vector getColumnMemberAssigneeData(Context context, String[] args) throws MatrixException
	{
		...
		try {
			...
			String ATTRIBUTE_PERCENT_ALLOCATION = PropertyUtil.getSchemaProperty(context,"attribute_PercentAllocation");
	...
	public Vector getTaskDelivarablesIcon(Context context, String[] args) throws MatrixException
	{
		...
		try
		{
			...
			String strLanguage = context.getSession().getLanguage();
			String SELECT_DELEVERABLE_POLICY = "from["+DomainConstants.RELATIONSHIP_TASK_DELIVERABLE+"].to.policy";
			String SELECT_DELEVERABLE = "from["+DomainConstants.RELATIONSHIP_TASK_DELIVERABLE+"].to.id";
			String SELECT_DELEVERABLE_NAME = "from["+DomainConstants.RELATIONSHIP_TASK_DELIVERABLE+"].to.name";
			String SELECT_DELEVERABLE_STATE = "from["+DomainConstants.RELATIONSHIP_TASK_DELIVERABLE+"].to.current";
			String SELECT_DELEVERABLE_MD = "from["+DomainConstants.RELATIONSHIP_TASK_DELIVERABLE+"].to.modified";
			...
			for (i = 0; i < actionListSize; i++)
			{
				...
				strToolTip = "";
				if(slTaskSubTypes.contains(strTaskType) || mileStoneSubtypeList.contains(strTaskType)&& DomainConstants.POLICY_PROJECT_TASK.equals(taskPolicy)){

					if(ProgramCentralUtil.isNotNullString(strDelvarables)){
						strDelNames = (String) mpObjDetails.get(SELECT_DELEVERABLE_NAME);
						slDelNames = StringUtil.splitString(strDelNames, matrix.db.SelectConstants.cSelectDelimiter);
						strDelState = (String) mpObjDetails.get(SELECT_DELEVERABLE_STATE);
						slDelState = StringUtil.splitString(strDelState, matrix.db.SelectConstants.cSelectDelimiter);
						strModifiedDate = (String) mpObjDetails.get(SELECT_DELEVERABLE_MD);
						slDelModDate = StringUtil.splitString(strModifiedDate, matrix.db.SelectConstants.cSelectDelimiter);
						strPolicy = (String) mpObjDetails.get(SELECT_DELEVERABLE_POLICY);
						slPolicy = StringUtil.splitString(strPolicy, matrix.db.SelectConstants.cSelectDelimiter);
	...
	public StringList isProjectTemplateSummaryTableColumnsEditable (Context context, String[] args) throws MatrixException
	{
		...
		if( (intLevel==0 && domObj.isKindOf(context,DomainConstants.TYPE_PROJECT_TEMPLATE)) )
		{
			slHasAccess.add(false);
		}
		else
	...
	public Map getPercentageRangeValues(Context context, String[] args)throws Exception
	{
		...
		try{
			...
			if(ProgramCentralUtil.isNotNullString(strObjectId)){
				DomainObject domObj = DomainObject.newInstance(context,strObjectId);
				String sObjectPolicy = domObj.getInfo(context, DomainConstants.SELECT_POLICY);
				StringList percentageList = new StringList();
	...
	public StringList excludeProjectsfromAddExisting(Context context, String[] args)throws MatrixException
	{
		...
		String strMasterProjectId=DomainConstants.EMPTY_STRING;
		...
		final String SELECT_MEMBER_ID = "from["+DomainConstants.RELATIONSHIP_MEMBER+"].to.id";
		...
		try {
			...
			String relPattern = DomainConstants.RELATIONSHIP_SUBTASK;
			String typePattern=DomainConstants.TYPE_PROJECT_MANAGEMENT;
			...
			String strRelatedPtojectTypes = DomainConstants.EMPTY_STRING;
			if(ProgramCentralConstants.TYPE_EXPERIMENT.equalsIgnoreCase(strType)){
				for(int i=0; i<mlRelatedProjects.size();i++){
					...
					strRelatedPtojectTypes = (String)mpRelatedProject.get(DomainConstants.SELECT_TYPE);

					if(strRelatedPtojectTypes.equalsIgnoreCase(ProgramCentralConstants.TYPE_EXPERIMENT)){
						busSelectsAll.addElement((String)mpRelatedProject.get(DomainConstants.SELECT_ID));
					}
				}
				...
				for (int i = 0; i < mlRelatedProjects.size(); i++){
					...
					if(strRelatedPtojectTypes.equalsIgnoreCase(DomainConstants.TYPE_PROJECT_SPACE)|| strRelatedPtojectTypes.equalsIgnoreCase(DomainConstants.TYPE_PROJECT_CONCEPT))
					{
						busSelectsAll.addElement((String)mpRelatedProject.get(DomainConstants.SELECT_ID));
					}
				}
			...
			if(!TYPE_PROJECT_CONCEPT.equalsIgnoreCase(strType)) { //When Project Concept is parent, another concept can be added as sub-project
				typeList.add(DomainConstants.TYPE_PROJECT_CONCEPT);
			}


			StringList stateList = new StringList(DomainConstants.STATE_PROJECT_SPACE_ARCHIVE);
			stateList.add(DomainConstants.STATE_PROJECT_SPACE_COMPLETE);
			...
			for(int i=0;i<mlProjectList.size();i++){
				...
				String strArchivePtojectId = (String)mpProject.get(DomainConstants.SELECT_ID);
				String owner = (String)mpProject.get(SELECT_OWNER);
				String strPtojectType = (String)mpProject.get(DomainConstants.SELECT_TYPE);
				String strProjectState = (String)mpProject.get(DomainConstants.SELECT_CURRENT);
	...
	public Map getTaskPolicyRange(Context context, String[] args)  throws Exception
	{
		...
		try{
			...
			StringList taskTypeList = StringUtil.split(taskType, ProgramCentralConstants.COMMA);
			...
			if (((String)taskTypeList.get(0)).contains("_selectedType:")){
	    		...
	    		{
				taskType = DomainConstants.TYPE_TASK;
			}
			}
 ...
 */
   public Vector getDefaultCalendarColumn(Context context, String[] args) throws Exception {
	   ...
	   if(objectList.size() > 0){
		   ...
		   if(ProgramCentralUtil.isNotNullString(parentProjectId)){
			   ...
			   Map projectInfoMap = project.getInfo(context, selectable);
			   ...
			   if("TRUE".equalsIgnoreCase(projectSpaceType) && (DomainConstants.STATE_PROJECT_SPACE_CREATE.equalsIgnoreCase(projectState) || DomainConstants.STATE_PROJECT_SPACE_ASSIGN.equalsIgnoreCase(projectState))){
	   ...
	   while(objectListIterator.hasNext()){
		   Map calendarMap = objectListIterator.next();
		   String sRowID = (String)calendarMap.get("id[level]");
		   String sOID = (String)calendarMap.get(DomainConstants.SELECT_ID);
		   String sRelId = (String)calendarMap.get(DomainRelationship.SELECT_ID);
		   String parentId = (String)calendarMap.get("id[parent]");
		   String relationshipName = (String)calendarMap.get(DomainConstants.KEY_RELATIONSHIP);
    ...
    */
    public boolean hasAccessToPMCContentSummaryToolBarActions(Context context, String args[]) throws Exception {
    	...
    	DomainObject domOBj = DomainObject.newInstance(context, objectId);
    	Map infoMap = domOBj.getInfo(context, selectables);
    		...
    		while(taskInfoIterator.hasNext()){
    			...
    			if("h".equalsIgnoreCase(durationInputUnit)){
    				...
    				if("false".equalsIgnoreCase(isTaskMagt)){
    					...
    					String workingTime = project.getInfo(context, SELECT_WORKING_TIME_PER_DAY);
                        ...
                        else if(hasConnectedRoutes(context, strTaskOID))
                        {
                            ...
                            if(newPercentValue >= 0 && newPercentValue < 100) {
                                ...
                                if ( taskReviewStatePosition == taskTargetPosition &&
                                    ...
                                    DomainConstants.POLICY_PROJECT_TASK.equalsIgnoreCase(taskPolicy)) {
...
public static Map getTaskManagementTypes(Context context, String[] args)throws Exception
        ...
        /*if(ProgramCentralConstants.POLICY_PROJECT_REVIEW.equalsIgnoreCase(sObjectPolicy))
        {
	        ...
	        if("true".equalsIgnoreCase(isSummaryTask)){
	        	StringList reviewTypeList = new StringList(2);
	        	reviewTypeList.addAll(ProgramCentralUtil.getSubTypesList(context, DomainConstants.TYPE_TASK));
