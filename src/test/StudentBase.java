...
public class ${CLASSNAME} extends com.matrixone.apps.program.Task
{
	...
	public void triggerModifyAttributes(Context context, String[] args)
		...
		if(sEvent.equals("Promote"))
		{
			...
			else if (toState.equals(STATE_PROJECT_TASK_COMPLETE))
			{
				...
				else if (Double.valueOf(estDuration).doubleValue() == 0)
				{
	...
	protected void calculatePercentComplete(Context context)
      ...
      //Added:12-Mar-10:vm3:R209:PRG:Bug 016043
        ...
        if(!parentTaskId.equals("") && action.equals("remove"))
        {
            ...
            dselectedTaskDuration = Double.parseDouble(selectedTaskDuration);
            dselectedTaskPercentComplete = Double.parseDouble(selectedTaskPercentComplete);
		...
		try
		{
			...
			Collections.sort(percentageList,new Comparator()
			{
				public int compare(Object o1,Object o2)
				{
					...
					double firstEntry=Double.parseDouble(first);
					double secondEntry=Double.parseDouble(second);
	...
	public int triggerChangeOwnerAction (Context context, String[] args)
        ...
        try{
            ...
            if (taskInfo.get(SELECT_SUBTASK_IDS) == null)
            {
                ...
                Long duration = new Long(Double.valueOf(sDuration).longValue());
	...
	public void updateTaskActualFinishDate(Context context, String[] args) throws Exception
	{
		...
		if (!strNewVal.equals(strOldVal)) {
			...
			if (Double.valueOf(strEstDuration).doubleValue() == 0)
			{
	...
	public void updateTaskPercentageComplete(Context context, String[] args) throws Exception
	{
        ...
        double oldPercentValue     = Double.parseDouble(oldPercent);
        double currentPercentValue = Double.parseDouble(strNewVal);
		...
		for(int j=0; j < bwsl.size(); j++)
		{
			...
			if (predecessorIds.size() > 0 && !"".equals(((String)predecessorIds.get(0)).trim()))
			{
				...
				for (int i=0; i < depedencyList.size(); i++)
				{
					...
					String href = null;
					if(prjSpaceSubTypes.indexOf(predecessorProjectType)>=0)
					{
						if (prjSpaceSubTypesHref == null)
						{
							prjSpaceSubTypesHref = UIMenu.getHRef(UIMenu.getCommand(context, "PMCWBS"));
							prjSpaceSubTypesHref = prjSpaceSubTypesHref.replace("${SUITE_DIR}","../programcentral");
							prjSpaceSubTypesHref = prjSpaceSubTypesHref.replace("&","&amp;");
							prjSpaceSubTypesHref = prjSpaceSubTypesHref + "&amp;suiteKey=ProgramCentral&amp;";
						}
						href = prjSpaceSubTypesHref;
					}
					else if (prjConceptSubTypes.indexOf(predecessorProjectType)>=0)
					{
						if (prjConceptSubTypesHref == null)
						{
							prjConceptSubTypesHref = UIMenu.getHRef(UIMenu.getMenu(context, "type_ProjectConcept"));
							prjConceptSubTypesHref = prjConceptSubTypesHref.replace("${COMMON_DIR}","../common");
							prjConceptSubTypesHref = prjConceptSubTypesHref.replace("&","&amp;");
							prjConceptSubTypesHref = prjConceptSubTypesHref + "&amp;suiteKey=ProgramCentral&amp;";
						}
						href = prjConceptSubTypesHref;
					}
					else if(prjTemplateSubTypes.indexOf(predecessorProjectType)>=0)
					{
						if (prjTemplateSubTypesHref == null)
						{
							prjTemplateSubTypesHref = UIMenu.getHRef(UIMenu.getMenu(context, "type_ProjectTemplate"));
							prjTemplateSubTypesHref = prjTemplateSubTypesHref.replace("${SUITE_DIR}","../programcentral");
							prjTemplateSubTypesHref = prjTemplateSubTypesHref.replace("${COMMON_DIR}","../common");
							prjTemplateSubTypesHref = prjTemplateSubTypesHref.replace("&","&amp;");
							prjTemplateSubTypesHref = prjTemplateSubTypesHref + "&amp;suiteKey=ProgramCentral&amp;";
						}
						href = prjTemplateSubTypesHref;
					}
					else
					{
					...
					tip += Double.valueOf(predecessorLagTime) < 0 ? "" : "+";
		...
		for(int j=0; j < bwsl.size(); j++)
		{
			...
			if (successorIds.size() > 0 && !"".equals(((String)successorIds.get(0)).trim()))
			{
				...
				for (int i=0; i < depedencyList.size(); i++)
				{
					...
					String href = null;
					if(prjSpaceSubTypes.indexOf(successorProjectType)>=0)
					{
						if (prjSpaceSubTypesHref == null)
						{
							prjSpaceSubTypesHref = UIMenu.getHRef(UIMenu.getCommand(context, "PMCWBS"));
					...
					else if (prjConceptSubTypes.indexOf(successorProjectType)>=0)
					{
						if (prjConceptSubTypesHref == null)
						{
							prjConceptSubTypesHref = UIMenu.getHRef(UIMenu.getMenu(context, "type_ProjectConcept"));
					...
					else if(prjTemplateSubTypes.indexOf(successorProjectType)>=0)
					{
						if (prjTemplateSubTypesHref == null)
						{
							prjTemplateSubTypesHref = UIMenu.getHRef(UIMenu.getMenu(context, "type_ProjectTemplate"));
					...
					tip += Double.valueOf(successorLagTime) < 0 ? "" : "+";
					...
					if (ProgramCentralConstants.STATE_PROJECT_SPACE_HOLD_CANCEL_HOLD.equals(externalProjectState)
						...
						tip = XSSUtil.encodeForHTML(context, tip);
						value.append("<a href ='javascript:showModalDialog(\"");
						value.append(href + "objectId=" + XSSUtil.encodeForURL(context,successorId));
						value.append("\", \"875\", \"550\", \"false\", \"popup\")'>");
						value.append(tip + "</a>");
					}
					toolTip.append(tip);
	...
	public StringList getTaskDependentProject(Context context, String[] args)throws Exception {
		...
		for(int i=0;i<taskProjectInfoMapList.size();i++) {
			...
			projectLink.append(XSSUtil.encodeForHTML(context,strTaskProjectName));
					...
					if(currentState.equals(STATE_PROJECT_TASK_CREATE) && !ProgramCentralConstants.POLICY_PROJECT_REVIEW.equals(taskPolicy))
						...
						MapList assigneeList = getAssignees(context, busSelects, null, null);
								...
								if (assigneeList.size() > 0)
								{
									...
									sMailSubject  = emxProgramCentralUtilClass.getMessage(
											context, sMailSubject, mKey, mValue, companyName);
									//get the mail message
									String sMailMessage =
									...
									String mValue1[] = {projectSpace, taskName, taskDescription, taskEstStartDate,
											taskEstFinishDate, mailToList.toString(), taskOwner, ""};
									sMailMessage  = emxProgramCentralUtilClass.getMessage(
											context, sMailMessage, mKey1, mValue1, companyName);
                        ...
                        if ( ProgramCentralUtil.isNotNullString(baseurl) &&
                                (objectIdList != null && objectIdList.size() != 0) )
                           {
                               ...
                               while (iterator.hasNext())
                               {
                            	   ...
                            	   sMailMessage += "\n" + baseurl + "?objectId=" + (String) iterator.next();

                               }

                           }

            			${CLASS:emxNotificationUtilBase}.sendJavaMail(context, mailToList, mailCcList, null,
            					sMailSubject, sMailMessage, null, rpeUserName, null, objectIdList, "both");
								} // if
				} //for
		...
		try {
			if(newValue!=null) {
				...
				while (predecessorItr.hasNext()) {
					...
					if(Double.parseDouble(strLagTime)<0) {
				...
				for (int i=0;i<newDependencyArray.length;i++) {
					...
					if(dependencyType.length() >2) {
						...
						if(lagTime.contains("h")){
							lagtimeUnit = "h";
						}
						slack = dependencyType.substring(2,lagTime.length());
						roundupLagTime = Double.valueOf(slack);
			...
			while(itrC.hasNext()){
    			...
    			if ("resequence".equals(markup)){
					...
					if(isToshowDependencyMsg) {
						String strParentTaskName = (String)parentTaskInfoMap.get(SELECT_NAME);
	...
	private Map validateCutPasteOperation(Context context, Element rootParent) throws FrameworkException {
		...
		for(int i = 0; (i < childList.size() && !stopSearch); i++){
	...
	protected MapList getWBSTasks(Context context, String objectId, String relPattern,short nExpandLevel) throws Exception
	{
		...
		try {
			...
			String rootNodeHasMarkedDel = (String) rootNodeTNR.get("to["+DomainConstants.RELATIONSHIP_DELETED_SUBTASK+"].id");
			if(rootNodeHasMarkedDel!=null)
			{
				return resultList;
			}
 ...
 throws Exception
 {
	...
	public void updateConstriantDate(Context context,String args[]) throws Exception{
		...
		if(ProgramCentralUtil.isNotNullString(strNewValOfDate)){       	
			strNewValOfDate = eMatrixDateFormat.getFormattedInputDate(context,strNewValOfDate,clientTZOffset, locale);
	...
	public boolean isPlanningView(Context context,String args[]) throws Exception
	{
		try {
			...
			String strToolBar = (String)inputMap.get("selectedTable");
			DomainObject dmoObject = DomainObject.newInstance(context,strObjectId);

			if(dmoObject.isKindOf(context, DomainConstants.TYPE_PROJECT_CONCEPT) || !strToolBar.equalsIgnoreCase("PMCWBSViewTable")){
				return true;
			}else{
				return false;
			}
	...
	public static MapList getPMCWBSPlanningViewTable(Context context, String[]args) throws FrameworkException{
		...
		try {
			...
			String strConstraintTypei18 ="";
			strConstraintTypei18 = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
					"emxProgramCentral.TaskConstriant.WBSPlanningView.ExcludeColumns", languageStr);
			...
			if("TRUE".equalsIgnoreCase(isKindofTaskManagement)) {
				...
				if(null!=strDurationKeyword && !"".equals(strDurationKeyword) && !"null".equals(strDurationKeyword)) {
					...
					if (unit != null)
					{
						...
						double nTaskEstDuration = Double.parseDouble(unit.denormalize(strDuration));
	...
	public void checkDurationKeyword(Context context, Map attributeMap, String objectId)throws FrameworkException
	{
		try
		{
			...
			if(null!=strDurationKeyword && !"".equals(strDurationKeyword) && !"null".equals(strDurationKeyword))
			{
				...
				try
				{
					nLagDuration =  Double.parseDouble(strLagDuration.substring(0,strLagDuration.indexOf(" ")));
	...
	public HashMap getPMCTaskPolicies(Context context,String[] args) throws Exception {
		...
		try{
			...
			if(task.isKindOf(context,ProgramCentralConstants.TYPE_GATE) || task.isKindOf(context,ProgramCentralConstants.TYPE_MILESTONE)){
	...
	public void  updateAllocation(Context context, String[] args) throws Exception{
		...
		//Else, if user has Project Lead access, the user is valid
			...
			String projectId = (String)taskInfoMap.get(SELECT_PROJECT_ID);
			busSelect.clear();
			busSelect = new StringList();
			busSelect.add(ProgramCentralConstants.SELECT_NAME);
			StringList relSelect = new StringList();
			String SELECT_PROJECT_ACCESS = "attribute[Project Access]";
			String strWhere = SELECT_PROJECT_ACCESS + " == \"" + "Project Lead" + "\" || " + SELECT_PROJECT_ACCESS + " == \"" + "Project Owner" +"\"";
			relSelect.add(SELECT_PROJECT_ACCESS);
			ProjectSpace project = new ProjectSpace(projectId);

			MapList members = project.getMembers(context, busSelect, relSelect, "", strWhere);
			for (Iterator itrMembers = members.iterator(); itrMembers.hasNext();) {
				Map memberInfoMap = (Map) itrMembers.next();
				String validMember = (String)memberInfoMap.get(SELECT_NAME);
				if(validMember.equals(strUser)){
					isContextUserValid = true;
					break;
				}
			}
	...
	public int triggerUpdateRemainingEffort(Context context, String[] args) throws MatrixException {
		...
		try {
			...
			if("TRUE".equalsIgnoreCase(isKindofTaskManagement) && !slEffortIds.isEmpty()) {
				...
				totalEffort = Double.parseDouble(totalEffortString); 

				if(ProgramCentralUtil.isNullString(totalpendingString)) {
					totalpendingString = "0.0";
				}
				total_pending = Double.parseDouble(totalpendingString); 
	...
	public StringList isDependencyCellsEditable (Context context, String[] args) throws Exception
	{
		...
		if(intLevel==0 && domObj.isKindOf(context,ProgramCentralConstants.TYPE_PROJECT_SPACE) || domObj.isKindOf(context,ProgramCentralConstants.TYPE_PROJECT_TEMPLATE) || domObj.isKindOf(context,ProgramCentralConstants.TYPE_PROJECT_CONCEPT))
		{
			slHasAccess.add(false);
		}
		else
		{
			// if the user clicks on a Shadow Gate and brings it up as root in the schedule we don't want it editable
			if (domObj.isKindOf(context,ProgramCentralConstants.TYPE_GATE))
			{
				strShadowGateId = domObj.getInfo(context, SELECT_SHADOW_GATE_ID);
		...
		Map objectMap = null;
		String[] objIdArr = new String[objectList.size() - 1];

		for(int ind=1;ind<objectList.size();ind++){
			objectMap = (Map)objectList.get(ind);
			objIdArr[ind - 1] = (String)objectMap.get(DomainObject.SELECT_ID);
		}
		...
		for (Iterator itrTableRows = mlTaskDetails.iterator(); itrTableRows.hasNext();)
		{
			...
			strShadowGateId = (String)map.get(SELECT_SHADOW_GATE_ID);
			if(strShadowGateId == null || strShadowGateId.equals("")){
				slHasAccess.add(true);
			}else{
				slHasAccess.add(false);
	...
	private double getTwoDigitDecimalValue(double dblValue){
		...
		double dblPersonsTotalEfforts = Double.parseDouble(twoDForm.format(dblValue));
	...
	public StringList excludeProjectsfromAddExisting(Context context, String[] args)throws MatrixException
	{
		...
		try {
			...
			typeList.add(ProgramCentralConstants.TYPE_PROJECT_BASELINE);
	...
	public StringList isTaskNameCellEditable(Context context, String[] args) throws MatrixException{
		try{
			...
			for (Iterator itrTableRows = taskInfoList.iterator(); itrTableRows.hasNext();){
				...
				if(!ProgramCentralUtil.isNullString(taskState)){
					if(ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE.equals(taskState) || ProgramCentralConstants.STATE_PROJECT_SPACE_ARCHIVE.equals(taskState)) {
						slResult.add(false);
						continue;
					}
				}
				...
				StringList slTaskSelects = new StringList();
				slTaskSelects.addElement(SELECT_CURRENT);
				slTaskSelects.addElement(SELECT_SHADOW_GATE_ID);
				Map mTaskInfo = task.getInfo(context,slTaskSelects);
				taskState = (String)mTaskInfo.get(SELECT_CURRENT);
				String sIsTaskAShadowGate  = (String)mTaskInfo.get(SELECT_SHADOW_GATE_ID);


				if(ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE.equals(taskState) || ProgramCentralConstants.STATE_PROJECT_SPACE_ARCHIVE.equals(taskState)
						|| (sIsTaskAShadowGate != null && !sIsTaskAShadowGate.equals("")))
					slResult.add(false);
				else
					slResult.add(true);
			}
	...
	public boolean hasAccessForTemplateTaskRMB(Context context, String args[]) throws Exception
	{
		...
		if(null !=strObjectId){
			...
			if(DomainConstants.TYPE_PROJECT_TEMPLATE.equalsIgnoreCase(isTempLate) && (DomainConstants.TYPE_TASK.equalsIgnoreCase(isTask) 
					|| "Phase".equalsIgnoreCase(isTask) || "Milestone".equalsIgnoreCase(isTask))){
				return false;
			}
		}
		return true;
 ...
 */
    ...
    */
    public boolean hasAccessForWBSRMBCommand(Context context, String args[]) throws Exception {
	   ...
	   if(dObj.isKindOf(context, TYPE_PROJECT_TEMPLATE)){
		   blAccess = false;
	   }
	   if(blAccess){
    	...
    	try
    	{
    		...
    		while(taskInfoIterator.hasNext()){
    			...
    			if("h".equalsIgnoreCase(durationInputUnit)){
    				double duration = Double.parseDouble(taskEstimatedDuration);
    ...
    /**
     * This method will check whether the task is summary task or not, if it's a summary task then the Estimated Duration cell will not be editable.
     * @param context  the eMatrix <code>Context</code> object
     * @param args holds the input arguments
