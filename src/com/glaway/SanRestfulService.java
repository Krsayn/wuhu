 ...
 **/
@Path("/SanRestful")
public class SanRestfulService extends RestService {

    /**
     * https://r2019x.glaway.com/3dspace/resources/dsic/SanRestful/sayHello?pid=58813.57799.19323.45771&tid=58813.57799.36701.50798
     * @Description
     * @Author Dumpling
     * @param request
    ...
    @GET
    @Path("/sayHello")
    public Response sayHello(@javax.ws.rs.core.Context HttpServletRequest request) throws  Exception
    {
        ...
        String objectId = request.getParameter("pid");;
        String strTypeName = "";
        ...
        String sTemplateId = request.getParameter("tid");;
        ...
        DomainObject elseObj = new DomainObject("58813.57799.57810.51269");
    ...
    /**
     * https://r2019x.glaway.com/3dspace/resources/dsic/SanRestful/routeTest?action=promote
     * @param request
     * @return
     * @throws Exception
     */
    @GET
    @Path("/routeTest")
    public Response routeTest(@javax.ws.rs.core.Context HttpServletRequest request) throws  Exception{
        Context context = Framework.getContext(request.getSession());

        String docId="58813.57799.5269.27026";
        DomainObject docObj = new DomainObject(docId);

        System.out.println("=======================");


        // get current state
        String current = docObj.getInfo(context, "current");
        ...
        if("promote".equals(action)){
            // state forward
            docObj.promote(context);
        }else{
            // state backward
            docObj.demote(context);
        }

        //"promote".equals(action)?docObj.promote(context):docObj.demote(context);

        return Response.ok("success routeTest").build();
    }
}
