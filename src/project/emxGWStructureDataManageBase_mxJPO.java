...
public Map createNewProduct(Context context,String[]args)throws Exception
        ...
        String shipClassification 			= (String)programMap.get("GWShipClassification");
        String flag 			= (String)programMap.get("GWFlag");
        String contractDate 			= (String)programMap.get("GWContractDate");
        String deliveryDate 			= (String)programMap.get("GWDeliveryDate");
        
        System.out.println("shipClassification=======>"+shipClassification);
		...
		String strTimeZone 				= (String)programMap.get("timeZone");
		  ...
		  if(ProgramCentralUtil.isNotNullString(contractDate)){
		        ...
		        contractDate = eMatrixDateFormat.getFormattedInputDateTime(contractDate, strInputTime, iDateFormat, dClientTimeZoneOffset, locale);
			}
		  

		  if(ProgramCentralUtil.isNotNullString(deliveryDate)){
				/*projectDate = projectDate.trim();
				projectDate = eMatrixDateFormat.getFormattedInputDate(context,projectDate,dClientTimeZoneOffset,locale);*/
				TimeZone tz = TimeZone.getTimeZone(context.getSession().getTimezone());
				double dbMilisecondsOffset = (double)(-1)*tz.getRawOffset();
				...
				deliveryDate = deliveryDate.trim();
				 int iDateFormat = eMatrixDateFormat.getEMatrixDisplayDateFormat();
		        String strInputTime = eMatrixDateFormat.adjustTimeStringForInputFormat("");
		        deliveryDate = eMatrixDateFormat.getFormattedInputDateTime(deliveryDate, strInputTime, iDateFormat, dClientTimeZoneOffset, locale);
			}
        ...
        product.setAttributeValue(context,"GWShipClassification",shipClassification);
        product.setAttributeValue(context,"GWFlag",flag);
        product.setAttributeValue(context,"GWContractDate",contractDate);
        product.setAttributeValue(context,"GWDeliveryDate",deliveryDate);
